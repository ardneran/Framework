...
Bound::Bound(const Vec3& center, const Vec3& extent)
: m_center(center)
, m_extent(extent) {
...
Bound::Bound(const float& minX, const float& minY, const float& minZ, const float& maxX, const float& maxY, const float& maxZ) {
    m_cornerMin.x = minX;
    m_cornerMin.y = minY;
    m_cornerMin.z = minZ;
    m_cornerMax.x = maxX;
...
void Bound::updateCenter(const Vec3& center) {
    m_center = center;
    m_cornerMin = center - m_extent;
    m_cornerMax = center + m_extent;
}

void Bound::updateCenterExtent(const Vec3& center, const Vec3& extent) {
    m_center = center;
    m_extent = extent;
    m_cornerMin = center - extent;
    m_cornerMax = center + extent;
...
void Bound::updateMinMax(const Vec3& cornerMin, const Vec3& cornerMax) {
...
void Bound::updateMinMax(const float& minX, const float& minY, const float& minZ, const float& maxX, const float& maxY, const float& maxZ) {
    m_cornerMin.x = minX;
    m_cornerMin.y = minY;
    m_cornerMin.z = minZ;
    m_cornerMax.x = maxX;
...
void Bound::create(const std::list<Bound>& boxes) {
    std::list<Vec3> points;
    for (std::list<Bound>::const_iterator boxPointer = boxes.begin(); boxPointer != boxes.end(); ++boxPointer) {
...
void Bound::create(const std::list<Vec3>& points) {
...
bool Bound::contains(const Vec3& p) const {
...
Bound Bound::transform(const Mat4& m) const {
...
Bound Bound::transform(const Transform& t) const {
