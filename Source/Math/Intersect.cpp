...
bool testIntersect(const Bound3& bound, const Frustum3& frustum) {
	...
	int out;
	out = 0; for(int i = 0; i < 8; ++i) out += ((frustum.m_points[i].x > (bound.getCornerMax().x + band)) ? 1 : 0); if (out == 8) return 0;
	out = 0; for(int i = 0; i < 8; ++i) out += ((frustum.m_points[i].x < (bound.getCornerMin().x - band)) ? 1 : 0); if (out == 8) return 0;
	out = 0; for(int i = 0; i < 8; ++i) out += ((frustum.m_points[i].y > (bound.getCornerMax().y + band)) ? 1 : 0); if (out == 8) return 0;
	out = 0; for(int i = 0; i < 8; ++i) out += ((frustum.m_points[i].y < (bound.getCornerMin().y - band)) ? 1 : 0); if (out == 8) return 0;
	out = 0; for(int i = 0; i < 8; ++i) out += ((frustum.m_points[i].z > (bound.getCornerMax().z + band)) ? 1 : 0); if (out == 8) return 0;
	out = 0; for(int i = 0; i < 8; ++i) out += ((frustum.m_points[i].z < (bound.getCornerMin().z - band)) ? 1 : 0); if (out == 8) return 0;
